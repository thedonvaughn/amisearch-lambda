...
def lambda_handler(event, context):
  ...
  virtualization_type = event['ResourceProperties']['VirtualizationType'] #Get AMI VirtualizationType
  root_device_type = event['ResourceProperties']['RootDeviceType'] #Get AMI RootDeviceType
  ...
  try:
    ...
    if len(images) == 0:
      ...
      sendResponse(event, context, 'FAILED', {'Error': 'No Images Found'})

    # Sort Images by name
    sorted_images = sorted(images, key = lambda k: k['Name'])

    # Since the Image names include a timestamp, the most recent AMI will be the last element
